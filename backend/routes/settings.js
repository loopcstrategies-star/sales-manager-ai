...
const patchSchema = Joi.object({
  ...
  sales: Joi.object({
    ...
    findContactsMax: Joi.number().integer().min(3).max(15),
    findContactsNeedsVerify: Joi.boolean(),
    batchFindCap: Joi.number().integer().min(10).max(50),
    bulkQueries: Joi.number().integer().min(1).max(8),
    perQuery: Joi.number().integer().min(1).max(12),
    scheduledFindEnabled: Joi.boolean(),
    scheduledFindHours: Joi.number().integer().min(6).max(48),
    fillPipelineOnImport: Joi.boolean(),
    defaultProspectRegion: Joi.string().max(40).allow(''),
